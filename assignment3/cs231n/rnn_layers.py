...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.dot(prev_h, Wh) + np.dot(x, Wx)
    next_h += b
...
def rnn_step_backward(dnext_h, cache):
    ...
    dout = dnext_h * (1 - np.square(next_h))     # (N,H) * I -> (N,H)
    
    dx = np.dot(dout, Wx.T)                     # (N,H) @ (D,H).T -> (N,D)
    dWx = np.dot(x.T, dout)                     # (N,D).T @ (N,H)-> (D,H)
    
    # why (H,H).T?????
    dprev_h = np.dot(dout, Wh.T)                # (N,H) @ (H,H).T -> (N, H)
    # why (prev_h.T, dout) instead of (dout.T, prev_h)? 
    dWh = np.dot(prev_h.T, dout)                # (N,H).T @ (N,H) @ -> (H,H)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    Note: N & T are at the same level! 
    ...
    # Modified from https://github.com/samlkrystof/Stanford-CS231n/blob/main/assignment3/cs231n/rnn_layers.py
    for i in range(T): 
        # Since N & T are at the same level, it should be x[:,i] instead of x[:][i].
        next_h, cache_element = rnn_step_forward(x[:,i], next_h, Wx, Wh, b) 
        h[:,i] = next_h
...
def rnn_backward(dh, cache):
    ...
    # copied from https://github.com/samlkrystof/Stanford-CS231n/blob/main/assignment3/cs231n/rnn_layers.py
    N, T, H = dh.shape
    D = cache[0][0].shape[1]
    dx = np.zeros((N, T, D))
    dWx = np.zeros((D, H))
...
def word_embedding_forward(x, W):
    ...
    # I don't get it: how can (V,D)[(N,T)] output (N,T,D)???
    out = W[x]  # (N.T,D) = (V,D)[(N,T)]
    cache = (x, W)
...
def word_embedding_backward(dout, cache):
    ...
    x, W = cache
    dW = np.zeros(W.shape)
    # I don't get it: why add (N,T,D) at the position of (N,T) of (V,D)???
    np.add.at(dW, x, dout)
...
def temporal_softmax_loss(x, y, mask, verbose=False):
    ...
    # since we operate over minibatches and different captions may have different lengths, 
